...
fork_mod_mono_server (apr_pool_t *pool, xsp_data *config)
{
  ...
  // Build the path based on OPENSHIFT_HOMEDIR for use in OpenShift
  server_path = getenv("OPENSHIFT_HOMEDIR");
	if (config->server_path && config->server_path [0])
		...
		switch (config->target_framework [0]) {
			...
			case '4':
				strcat(server_path, "/mono/mono/bin/mod-mono-server4");
				break;
